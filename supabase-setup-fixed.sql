-- WexCars Database Fixed Setup Script
...
-- FIXED: Simple users table policies without recursion
...
-- Admin access using auth.jwt() instead of table lookup to avoid recursion
...
-- Admin users can insert any car (using email check to avoid recursion)
...
-- Admin users can update any car (using email check to avoid recursion)
...
-- Admin users can delete any car (using email check to avoid recursion)
