...
-- This is a simpler approach that logs new subscriptions for manual email sending
-- Run this after newsletter-quick-setup.sql
...
-- View to see pending notifications (for admin dashboard)
...
RETURNS void AS $$
BEGIN
  UPDATE newsletter_subscription_notifications
  SET status = 'notified',
      notified_at = NOW()
...
SELECT 'Simple email notification system created!' as status;
SELECT 'You can now query pending_subscription_notifications view to see new subscribers' as note;
